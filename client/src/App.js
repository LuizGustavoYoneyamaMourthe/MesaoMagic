...
import { addCardToDeck, createDeck, deleteDeck, getDeck, getDecks, removeDeckCard, searchCards, updateDeckCardQty, updateDeckName } from './api';
import { averageCmc, buildCurve, canBeCommander, colorIdentity, countColorPips, countLands, countZone, getDeckTarget, groupDeckCards, pickDefaultPreview, toPreviewCard, totalCards } from './deck-builder';
...
export default function App() {
    ...
    async function loadDeck(id, nextScreen) {
    ...
    async function refreshActiveDeck(nextScreen) {
        if (!activeDeckId) {
            return;
        }
    ...
    async function handleChangeQty(card, nextQty) {
        if (!activeDeckId) {
            return;
        }
        setLoadingDeckAction(true);
    ...
    return (_jsxs(_Fragment, { children: [_jsx("header", { className: "topbar", children: _jsxs("div", { className: "topbar__inner", children: [_jsxs("button", { className: "brand", type: "button", onClick: () => setScreen('dashboard'), children: [_jsx(ColorSymbols, { className: "brand-pips", colors: ['W', 'U', 'B', 'R', 'G'], size: "sm" }), _jsx("span", { className: "brand-title", children: "Mes\u00E3o do Dujour" }), _jsx("span", { className: "brand-subtitle", children: "Construtor de Decks" })] }), _jsxs("div", { className: "topbar-actions", children: [screen === 'editor' ? (_jsxs(_Fragment, { children: [_jsx("button", { className: "secondary-button", onClick: () => setScreen('dashboard'), children: "\u2190 Decks" }), _jsx("button", { onClick: () => setScreen('viewer'), disabled: !activeDeck, children: "Visualizar" })] })) : null, screen === 'viewer' ? (_jsxs(_Fragment, { children: [_jsx("button", { className: "secondary-button", onClick: () => setScreen('dashboard'), children: "\u2190 Decks" }), _jsx("button", { onClick: () => setScreen('editor'), disabled: !activeDeck, children: "Editar deck" })] })) : null] })] }) }), _jsxs("main", { className: `app-shell${isWorkspaceScreen ? ' app-shell--workspace' : ''}`, children: [screen === 'dashboard' ? (_jsx(DeckDashboard, { decks: decks, loadingDecks: loadingDecks, onOpenCreate: () => setShowCreateModal(true), onOpenEditor: (deckId) => void loadDeck(deckId, 'editor'), onOpenViewer: (deckId) => void loadDeck(deckId, 'viewer'), onDeleteDeck: (deckId) => void handleDeleteDeck(deckId) })) : null, screen === 'editor' ? (_jsx(DeckEditor, { activeDeck: activeDeck, groupedCards: groupedCards, activeCards: activeCards, commanderCard: commanderCard, deckNameDraft: deckNameDraft, editorMode: editorMode, loadingSearch: loadingSearch, loadingDeckAction: loadingDeckAction, selectedZone: selectedZone, query: query, searchTotal: searchTotal, searchResults: searchResults, previewCard: previewCard, metrics: metrics, onChangeQuery: setQuery, onChangeDeckNameDraft: setDeckNameDraft, onSaveDeckName: () => void handleRenameDeck(), onSetEditorMode: setEditorMode, onSetZone: setSelectedZone, onHoverCard: setPreviewCard, onAddCard: (card) => void handleAddCard(card), onSetCommander: (card) => void handleSetCommander(card), onClearCommander: () => void handleClearCommander(), onChangeQty: (card, nextQty) => void handleChangeQty(card, nextQty), onRemoveCard: (card) => void handleRemoveCard(card) })) : null, screen === 'viewer' ? (_jsx(DeckViewer, { activeDeck: activeDeck, activeCards: activeCards, commanderCard: commanderCard, groupedCards: groupedCards, viewerMode: viewerMode, previewCard: previewCard, metrics: metrics, onSetViewerMode: setViewerMode, onHoverCard: setPreviewCard })) : null, showCreateModal ? (_jsx(CreateDeckModal, { loading: loadingDeckAction, newDeckName: newDeckName, newDeckFormat: newDeckFormat, onClose: () => setShowCreateModal(false), onChangeName: setNewDeckName, onChangeFormat: setNewDeckFormat, onCreate: () => void handleCreateDeck() })) : null, message ? _jsx("div", { className: "message-banner", children: message }) : null] })] }));
}
